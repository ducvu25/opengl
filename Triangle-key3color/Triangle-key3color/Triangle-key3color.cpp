...
"color= ourColor;\n"
"}\0";
GLfloat redValue = 1.0;
GLfloat greenValue = 0.5f;
GLfloat blueValue = 0.5f;
int main(void)
{
	...
	GLFWwindow* window = glfwCreateWindow(WIDTH, HEIGHT, "Bai thuc hanh so 3- doi mau", nullptr, nullptr);
	if (window == nullptr)
	{
	...
	glBindBuffer(GL_ARRAY_BUFFER, 0); //unbind VBO , cho phÃ©p gá»i hÃ m glVertexAttribPointer trong VBO
//unbind VAO
	glBindVertexArray(0);
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//Buoc 3 Váº½ hÃ¬nh 1 hÃ¬nh tam giÃ¡c
	...
	//Terminate GLFW, xoá và dọn dẹp tài nguyên sau khi thoát
	glfwTerminate();
	return 0;
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mode)
{
	...
	//else {
	//	redValue = (float)rand() / RAND_MAX;
	//	greenValue = (float)rand() / RAND_MAX;
	//	blueValue = (float)rand() / RAND_MAX;
	//}
}